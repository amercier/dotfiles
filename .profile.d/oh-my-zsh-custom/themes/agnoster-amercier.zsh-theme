...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ -n ${remote} ]] ; then
      local counts="$(git rev-list --count --left-right "$remote"...HEAD 2>/dev/null)"
    ...
    local stash="$(git stash list | wc -l | sed s'/[[:blank:]]//g')"
    if [[ $stash != 0 ]] ; then
      count="${count}≣${stash}"
    fi
    ...
    vcs_info
    echo -n "${ref/refs\/heads\//$PL_BRANCH_CHAR }${count}${vcs_info_msg_0_%% }${mode}"
  fi
}
